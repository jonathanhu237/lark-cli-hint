...
describe("docs workflow analyzer", () => {
  ...
  it("suggests docs +fetch --doc for real docs search result_meta URLs", () => {
    const t = createTranslator("en-US");
    const hint = analyze(result({
      command: ["lark-cli", "docs", "+search", "--as", "user", "--query", "Lark CLI Hint Test"],
      stdout: fixture("docs-search.real-result-meta-url.json")
    }), t);

    expect(hint.kind).toBe("success");
    expect(hint.status).toContain("Lark CLI Hint Test 20260504-053350");
    expect(hint.status).not.toContain("<h>");
    expect(hint.next.command).toBe("lark-cli docs +fetch --doc https://example.feishu.cn/docx/FXvYdujnGo5KNjxIId6ccyVunyh");
  });

  it("suggests docs +fetch --doc for real docs search result_meta tokens", () => {
    const t = createTranslator("en-US");
    const hint = analyze(result({
      command: ["lark-cli", "docs", "+search", "--as", "user", "--query", "Lark CLI Hint"],
      stdout: fixture("docs-search.real-result-meta-token.json")
    }), t);

    expect(hint.kind).toBe("success");
    expect(hint.status).toContain("Lark CLI Hint Token Only");
    expect(hint.next.command).toBe("lark-cli docs +fetch --doc FXvYdujnGo5KNjxIId6ccyVunyh");
  });
  ...
  it("recovers docs +search unsupported identity failures", () => {
    const t = createTranslator("en-US");
    const hint = analyze(result({
      command: ["lark-cli", "docs", "+search", "--query", "demo"],
      exitCode: 2,
      stdout: fixture("docs-search.identity-error.json")
    }), t);

    expect(hint.kind).toBe("failure");
    expect(hint.status).toContain("selected identity is unsupported");
    expect(hint.next.command).toBe("lark-cli docs +search --as user --query demo");
    expect(hint.sources.map((source) => source.type)).toContain("stdout");
  });

  it("replaces existing docs +search identity with user", () => {
    const t = createTranslator("en-US");
    const hint = analyze(result({
      command: ["lark-cli", "docs", "+search", "--as", "bot", "--query", "demo"],
      exitCode: 2,
      stdout: fixture("docs-search.identity-error.json")
    }), t);

    expect(hint.next.command).toBe("lark-cli docs +search --as user --query demo");
    expect(hint.next.command).not.toContain("--as bot");
  });

  it("preserves and quotes original docs +search arguments when recovering identity", () => {
    const t = createTranslator("en-US");
    const hint = analyze(result({
      command: [
        "lark-cli",
    ...
    expect(hint.next.command).toBe(
      "lark-cli docs +search --as user --query \"AI challenge\" --page-size 20 --format pretty --jq \".data.items[0]\""
    );
  });

  it("does not invent docs +search identity recovery without user identity evidence", () => {
    const t = createTranslator("en-US");
    const hint = analyze(result({
      command: ["lark-cli", "docs", "+search", "--query", "demo"],
      exitCode: 1,
      stderr: "search backend failed"
    }), t);

    expect(hint.kind).toBe("failure");
    expect(hint.next.command).toBeNull();
    expect(hint.status).toBe("Command failed with exit code 1.");
  });
  ...
  it("recovers unsupported identity docs fetch failures", () => {
    ...
    expect(hint.next.command).toBe(
      "lark-cli docs +fetch --as user --doc doccn_demo_project_brief --format pretty --jq .data.title"
    );
  });
  ...
  it("localizes unsupported identity recover values", () => {
    const t = createTranslator("zh-CN");
    const runResult = result({
      command: ["lark-cli", "docs", "+search", "--query", "演示"],
      exitCode: 2,
      stdout: fixture("docs-search.identity-error.json")
    });
    const hint = analyze(runResult, t);
    ...
    expect(envelope.hint.status).toContain("当前身份不受支持");
    expect(envelope.hint.next.command).toBe("lark-cli docs +search --as user --query \"演示\"");
    expect(JSON.stringify(envelope)).toContain("\"exitCode\"");
    expect(JSON.stringify(envelope)).not.toContain("\"退出码\"");
    expect(card).toContain("状态");
    expect(card).toContain("lark-cli docs +search --as user --query \"演示\"");
  });
});
