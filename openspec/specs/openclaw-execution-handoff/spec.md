# openclaw-execution-handoff Specification

## Purpose
`openclaw-execution-handoff` covers the default post-card OpenClaw handoff for `lark-cue`: verifying OpenClaw availability before a wrapped command runs, generating a deterministic task message from the failed command and Feishu-backed card, invoking the local `main` agent, streaming OpenClaw output to stderr, and preserving wrapped command exit semantics.
